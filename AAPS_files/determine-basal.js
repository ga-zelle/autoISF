...
{   // #### gz mod 7:  dynamic ISF strengthening based on duration and width of 5% BG band
    // #### gz mod 7b: misuse autosens_min to get the scale factor
    // #### gz mod 7d: use standalone variables for autopISF
    var dura05 = glucose_status.autoISF_duration;           // mod gz7d
    var avg05  = glucose_status.autoISF_average;            // mod gz7d
    //r weightISF = (1 - profile.autosens_min)*2;           // mod gz7b: use 0.6 to get factor 0.8; use 1 to get factor 0, i.e. OFF
    var weightISF = profile.autoisf_hourlychange;           // mod gz7d: specify factor directly; use factor 0 to shut autoISF OFF
    if (meal_data.mealCOB==0 && dura05>=10) {
        if (avg05 > target_bg) {
            // # fight the resistance at high levels
            var maxISFReduction = profile.autoisf_max;      // mod gz7d
            ...
            console.error("gz ISF", sens, "did not do it for", dura05,"m; go more aggressive by", round(levelISF,2));
            if (maxISFReduction < levelISF) {
                console.error("gz ISF reduction", round(levelISF,2), "limited by autoisf_max", maxISFReduction);
            }
        ...
        console.error("gz keep ISF as BG is only "+dura05+"m at level "+avg05);
    }
    return sens;
}
...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, isSaveCgmSource) {
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if (smb_max_range > 1) {
                    console.error("gz SMB max range extended from default by factor", smb_max_range)
            }
            ...
            if ( smb_ratio > 0.5) {
                console.error("gz SMB ratio increased from default 0.5 to", smb_ratio)
            }
