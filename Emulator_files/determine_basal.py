...
def autoISF(sens, target_bg, profile, glucose_status, meal_data, autosens_data, sensitivityRatio, new_parameter, Fcasts, Flows): 
    ...
    if meal_data['mealCOB'] > 0:                                # no action while digesting carbs
        console_error("gz keep ISF due to mealCOB of  "+str(round(meal_data['mealCOB'],1)) )
    ...
    if 'dura05' in glucose_status:                              # the AAPS 2.7 method
        ...
        dura70 = 0
    if avg05<target_bg :
        console_error("gz keep ISF; avg. glucose", avg05, "below target", target_bg)
    ...
    if dura70<10 and dura05<10 :
        ...
        console_error("gz keep ISF as BG is only "+str(short(dura05))+"m at level",short(avg05))
        Fcasts['emulISF'] = sens
        return sens
    ...
    if dura05>=10 and new_parameter['autoISF_flat']:            # long lasting at level
        ...
        if avg05 > target_bg:                                   # fight the resistance at high levels
            ...
            console_error('gz ISF', sens, 'did not do it for', dura05,'m; go', whereto,'aggressive by', str(round(levelISF,2))+msg_ce)
...
def determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctionsDummy, microBolusAllowed, reservoir_data, thisTime, Fcasts, Flows):
    ...
    sens = autoISF(sens, target_bg, profile, glucose_status, meal_data, autosens_data, sensitivityRatio, new_parameter, Fcasts, Flows)
    ...
    if ( minutesAboveThreshold < 240 or minutesAboveMinBG < 60 ) :
        ...
        Flows.append(dict(title="BG projected to remain above "+str(convert_bg(threshold,profile))+" for "+str(minutesAboveThreshold)+" minutes", indent='+1', adr='930'))
