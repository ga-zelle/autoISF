...
def TreatLoop(Curly, log, lcount):
    ...
    global longDelta, avgDelta, longSlope, rateSlope, glucose_status, origISF, emulISF
...
def get_glucose_status(lcount, st) :                    # key = 80
    ...
    if len(bg)==len(loop_mills) :
        ...
        bgTime.append(glucose_status['date']/1000/60)   # time of bg value in minutes
    else:
        bg[-1] = (glucose_status['glucose'])            # overwrite as last loop was not finished
        bgTime[-1] = (glucose_status['date']/1000/60)   # time of bg value in minutes
...
def getHistBG(iFrame, bw):
    ...
    for i in range(iFrame, -1, -1):
        if bgTime[iFrame] - bgTime[i] - duramins > 13 :    break   # at least 13 mins without CGM
        if bg[i]>oldavg*(1-bw) and bg[i]<oldavg*(1+bw) :            # still in previous bw ratio range
            sumBG += bg[i]
            oldavg = sumBG / (iFrame-i+1)
            duramins = ( bgTime[iFrame] - bgTime[i] )
...
##  y = a*x^2  + b*x  + c       respectively
    ...
    for i in range(iFrame, -1, -1):
        #if loop_mills[iFrame] - loop_mills[i] > 45*60:      break              # range no loonger than 45m
        ti     = loop_mills[i] -loop_mills[iFrame]         # time offset to make the numbers smaller for numerical accuracy
        ...
        if D != 0.0:  
            ...
            for j in range(i, iFrame+1):
                ...
                s_residual_squares += (bg[j] - (a*(loop_mills[j]-loop_mills[iFrame])**2 + b*(loop_mills[j]-loop_mills[iFrame]) + c))**2
            ...
            dur     = -ti/60    #(loop_mills[iFrame]-loop_mills[i])/60
            #print(loop_label[i], 'with', str(n), 'elements has   a =', str(a),' b=', str(b), ' c=', str(c))
            #print (i, loop_label[i], str(loop_mills[i]), str(bg[i]), str(dur))
                ...
                if featured('bg') :                                                     # plot bg
                    ...
                    if iFrame>2 and (featured('bestParabola') or featured('fitsParabola')): # show parabolas
                        ...
                        for i in parabs:
                            ...
                            tx = loop_mills[iFrame] +5*60                               # window end time = +5min from now
                            while tx >= loop_mills[iFrame]-dur*60:
                                ti = tx - loop_mills[iFrame]
                                bfit.append(a2*pow(ti,2) + a1*ti + a0)
                                tfit.append(ti+loop_mills[iFrame])
...
def parameters_known(myseek, arg2, variantFile, startLabel, stoppLabel, entries):
    ...
    if loopCount > 0 :   # ---   save the results from current logfile   --------------
        ...
        head= "    ----time formated as---       -----target-----                     -Autosens-   --5% range--    --lin.fit--   ---------ISFs---------    insulin Req   -maxBolus-   ---SMB---   ---tmpBasal---\n" \
            + " id    UTC         UNIX       bg    orig     emul    cob   iob    act  orig  emul   dura    avg     dura    avg   orig  prof  auto  emul    orig   emul    orig emul   orig emul     orig    emul"
        #print('\n' + head)
        xyf.write(head + '\n')
        ...
        xyf.write(tabz + '\n')
        tabz = 'Totals:'+ f'{round(origSMBsum,1):>164} {round(emulSMBsum,1):>4} {round(origBasalint,2):>9} {round(emulBasalint,2):>6}'
        xyf.write(sepLine + tabz + '\n' + sepLine)
    xyf.close()
