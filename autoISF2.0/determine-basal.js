...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (bg_off > 0) {
        ...
        console.error("delta_ISF adaptation by-passed as no rise or too short lived");
         ...
         var delta_ISF = interpolate(bg_delta, profile);
    ...
    if ( sens_modified ) {
        var liftISF = Math.max(Math.min(maxISFReduction, Math.max(levelISF, bg_ISF, delta_ISF)), sensitivityRatio);  // corrected logic on 30.Jan.2021
...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, isSaveCgmSource) {
    ...
    console.error('Meal age is:', (currentTime - meal_data.lastCarbTime) / 60000, 'minutes');
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if (bg < max_bg+10) {
                ...
                console.error("SMB reduced by " + 100*lessSMBRatio + "% because bg("+bg+") is below upper target("+max_bg+")+10");
            }
